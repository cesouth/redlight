...
class HMMMatcher:
    ...
    def _net_dist(self, src_node, dst_node, cutoff):
        # Bounded Dijkstra: respect the cutoff so max_route_dist_factor actually
        # prunes absurd detours (the previous unbounded call ignored it and ran a
        # full shortest-path search for every candidate transition).
        try:
            lengths = nx.single_source_dijkstra_path_length(
                self.network.graph, src_node, cutoff=cutoff, weight="length_m",
            )
        except nx.NodeNotFound:
            return None
        return lengths.get(dst_node)
    ...
    def _match_one(self, tid, sub) -> pd.DataFrame:
        ...
        for i in range(1, n):
            ...
            for eid, dist, _t in cur:
                ...
                v_dst, _ = self.network.edge_endpoints(eid)
                for peid, plp in V[i - 1].items():
                    pu, pv = self.network.edge_endpoints(peid)
                    rd = self._net_dist(pv, v_dst, cutoff)
                    if rd is None:
                        continue
                    delta = abs(step[i] - rd)
                ...
                if best_prev is None:
                    # no reachable predecessor; restart chain at this state
                    lp = em
                    best_lp, best_prev = lp, None
                V[i][eid] = best_lp
