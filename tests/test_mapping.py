...
def _annotate(net, *, peak=4.0, offpeak=16.0):
    ...
    for h, sp in ((8, peak), (22, offpeak)):
        rows += [{"edge_id": 0, "speed_mps": sp,
                 ...
                 for k in range(3)]
    rt.assign_segment_speeds(net, pd.DataFrame(rows), peak_hours=[8],
                             offpeak_hours=[22])


def test_to_geojson_units_and_alias(straight_net):
    _annotate(straight_net)
    fc = rt.to_geojson(straight_net, speed_unit="km/h")
...
def test_to_geojson_unknown_unit_raises(straight_net):
    ...
    with pytest.raises(ValueError, match="Unknown speed unit"):
        rt.to_geojson(straight_net, speed_unit="furlongs")


def test_to_geojson_period_export(straight_net):
    _annotate(straight_net)
    peak = rt.to_geojson(straight_net, period="peak", speed_unit="mps")
    off = rt.to_geojson(straight_net, period="offpeak", speed_unit="mps")
    ...
    with pytest.raises(ValueError, match="period"):
        rt.to_geojson(straight_net, period="rush")
...
def test_to_geojson_travel_time_consistent(straight_net):
    ...
    _annotate(straight_net)
    props = rt.to_geojson(straight_net, speed_unit="mps")["features"][0]["properties"]
...
def test_to_geojson_one_feature_per_road(straight_net):
    _annotate(straight_net)
    fc = rt.to_geojson(straight_net)  # non-directional
    assert len(fc["features"]) == 1
    assert sorted(fc["features"][0]["properties"]["edge_ids"]) == [0, 1]
    fc_dir = rt.to_geojson(straight_net, directional=True)
...
def test_to_geojson_writes_file(straight_net, tmp_path):
    ...
    rt.to_geojson(straight_net, str(out))
...
def test_plot_speed_map_smoke(straight_net, tmp_path):
    ...
    fig = rt.plot_speed_map(straight_net, str(out), period="peak")
    assert out.exists() and out.stat().st_size > 0
