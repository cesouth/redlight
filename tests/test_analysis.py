...
def test_missing_weight_attribute_raises(straight_net):
    ...
    with pytest.raises(ValueError, match="travel_time_s"):
        rl.edge_betweenness_centrality(straight_net, weight="travel_time_s")
...
def test_partial_missing_weight_still_raises(straight_net):
    ...
    rl.assign_speeds(straight_net, df)  # no default_speed_mps: only eid0 gets it
    with pytest.raises(ValueError, match="travel_time_s"):
        rl.edge_betweenness_centrality(straight_net, weight="travel_time_s")
...
def test_weight_length_m_works_without_any_pipeline(grid_net):
    bc = rl.edge_betweenness_centrality(grid_net, weight="length_m")
...
def test_weight_none_is_unweighted(straight_net):
    bc = rl.edge_betweenness_centrality(straight_net, weight=None)
...
def test_parallel_edges_favor_the_cheaper_one(tmp_path):
    ...
    bc = rl.edge_betweenness_centrality(net, weight="length_m")
...
def test_k_zero_raises_value_error_not_zero_division(straight_net):
    ...
    with pytest.raises(ValueError, match="k"):
        rl.edge_betweenness_centrality(straight_net, weight="length_m", k=0)


def test_k_sampling_reproducible_with_seed(grid_net):
    a = rl.edge_betweenness_centrality(grid_net, weight="length_m", k=5, seed=1)
    b = rl.edge_betweenness_centrality(grid_net, weight="length_m", k=5, seed=1)
    assert a == b


def test_write_attr_writes_onto_graph_only_when_given(straight_net):
    bc = rl.edge_betweenness_centrality(straight_net, weight="length_m")
    for eid in bc:
        assert "bc_test" not in straight_net.edge_data(eid)

    bc2 = rl.edge_betweenness_centrality(straight_net, weight="length_m",
                                         write_attr="bc_test")
...
def test_write_attr_collision_raises(straight_net):
    with pytest.raises(ValueError, match="length_m"):
        rl.edge_betweenness_centrality(straight_net, weight="length_m",
                                       write_attr="length_m")
    with pytest.raises(ValueError, match="travel_time_s"):
        rl.edge_betweenness_centrality(straight_net, weight="length_m",
...
def test_intersection_definition_collinear_vs_branching(tmp_path):
    ...
    net = rl.Network.from_geojson(collinear)
    ...
    net2 = rl.Network.from_geojson(branch)
    stats2 = rl.network_stats(net2)
...
def test_grid_is_strongly_connected(grid_net):
    cr = rl.connectivity_report(grid_net)
